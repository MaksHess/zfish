...
FEATURE_SETS = {
    ...
    "ccp-cens-asympt": (),  # No linear predictors
    "ccp-cens-power": (),  # No linear predictors
    "ccp-feats": (
        "PCNA",
        "DAPI",
        "n_Volume",
...
RETRIES = 5
PLOT_TOP_N_MODELS = [len(MODEL_FORMULA_PRED)]
CYCLES = [8, 9, 10, 11, 12]
SAMPLES = 50_000
SCALER = "standard"

OUT_PATH = Path(
    rf"C:\Users\hessm\Documents\Programming\Python\thesis\Figures\Results\Transcription\ModelResultsCCP\samples={SAMPLES}_scaler={SCALER}"
)
OUT_PATH.mkdir(exist_ok=True, parents=True)

CCP_QUICKSAVE = Path(
    r"C:\Users\hessm\Documents\Programming\Python\thesis\Data\for_model.parquet"
)

WITH_AND_WITHOUT_S5P = False

CENSORED = (np.log(2), None)
...
def main():
    ...
    df_all, df_s5p, df_no_s5p = compute_comparisions(
        loos, with_and_without_s5p=WITH_AND_WITHOUT_S5P
    )
        ...
        for cy in all_cycles:
            if WITH_AND_WITHOUT_S5P:
                ...
                az.plot_forest(
                    ...
                    model_names=[
                        k
                        for k in list(df_s5p[cy].index[:n])
                        + list(df_no_s5p[cy].index[:n])
                        if k not in ["ccp-asympt", "ccp-power"]
                    ][::-1],
                    filter_vars="like",
                    ax=ax,
                ...
                fig, axs = plt.subplots(
                    figsize=(2.6, n / 12 * 2 + 1), nrows=2, sharex=True
                )
                ...
                fig, ax = plt.subplots(figsize=(2.6, n / 12 + 1), nrows=1, sharex=True)
                az.plot_compare(df_all[cy].head(n), ax=ax, textsize=5.0)
                ax.set_ylabel("")
                ax.set_title(f"Interphase Models Cycle {cy}\nhigher is better")
...
def _run_asymtotic_and_linear_model(
    ...
    with pm.Model(coords=coords) as model:
        ...
        if censored is None:
            sample_kwargs = DEFAULT_FIT_PARAMS
            ...
            sample_kwargs = CENSORED_FIT_PARAMS
...
def plot_ccp_features(df, cycle=11, output_path=tpaths.transcription):
