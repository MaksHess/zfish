...
def _convert_itk_bbx(
    ...
    columns = {}
    for idx, dim in zip(bbx.GetIndex(), dims):
        columns[f"lower-{dim}"] = idx
    for idx, dim in zip(bbx.GetUpperIndex(), dims):
...
def _convert_itk_point_vector_index(
    ...
    return {d: p for d, p in zip(dims, point)}
...
def _convert_itk_matrix(
    ...
    columns = {}
    for m, dim in zip(np.array(matrix), outer_dims):
