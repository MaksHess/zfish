...
from zfish.features.polars_preprocessing import AnyFrameT
from zfish.features.polars_utils import nest, nest_selector, unnest
...
# TODO: Add nesting to (lazy) frame namespace
@pl.api.register_lazyframe_namespace("nest")
@pl.api.register_dataframe_namespace("nest")
class NestColumnsToStructs:
    def __init__(
        self,
        df: AnyFrameT
    ):
    ...
    def __call__(
        self,
        column_selector: SelectorType = cs.all(),
        on: str = ".",
    ):
        splittable_columns = cs.expand_selector(self._df, column_selector)



@pl.api.register_lazyframe_namespace("idx")
...
class IndexAccessor:
    ...
    def columns(self) -> list[str]:
        return (
            self._df.lazy()
            .select(pl.col(self._active_pattern), pl.col(self._inactive_pattern))
            .columns
        )
    ...
    def intersection(self, other: pl.DataFrame):
        ...
        return sorted(
            self.active_set.intersection(other.idx.active_set), key=order.index
        )
    ...
    def sort(self) -> pl.DataFrame:
        return self._df.select(
            pl.col(sorted(self.active, key=INDEX_ORDER)),
            pl.exclude(self.columns),
            pl.col(self.inactive),
        )
    ...
    def set_index(self, columns: str | Sequence[str] | pl.Expr) -> pl.DataFrame:
        ...
        columns_inactive_name = [
            f"_{column}" if not column.startswith("_") else f"{column}"
