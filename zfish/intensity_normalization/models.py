...
@dataclass
class Model(ABC):
    """Subclasses need to call both X, y = super().validate(X, y) and super().fit(X, y) in their fit method."""
    ...
    def predict(self, X: NDArray[Any]) -> NDArray[Any]:
        return CALLABLES[self._callable_key](np.asarray(X), *self.params)
    ...
    def _correction_factor(self, X: NDArray[Any]) -> NDArray[Any]:
        return 1 / self.predict(np.asarray(X)) * self.y_intercept
    ...
    def correct(self, X: NDArray[Any], y: NDArray[Any]) -> NDArray[Any]:
        return np.asarray(y) * self._correction_factor(np.asarray(X))
    ...
    def save(
        self, file_name: str | None = None, directory: str | Path = Path("."), mode="xb", write_params_json=True
    ) -> None:
    ...
    def __str__(self) -> str:
        class_name = self.__class__.__name__
        repr_fields = {k: getattr(self, k) for k, v in self.__dataclass_fields__.items() if v.repr==True}
        out = class_name + "(" + ', '.join([f"{_package_repr_key(k)}={_package_repr_value(v)}" for k, v in repr_fields.items()]) + ')'
        return out
        # if self._params is None:
        #     return f"{class_name}(loss={self.loss!r}): not fitted."
        # return f"{class_name}(loss={self.loss!r}, features={self._feature_names})"

def _numpy_to_list(arr) -> list:
    if isinstance(arr, np.ndarray):
        return list(arr)
    return arr

def _package_repr_value(obj: Any, sep=';') -> str:
...
class Linear(Model):
    ...
    def fit(self, X: NDArray[Any], y: NDArray[Any]) -> Self:
        ...
        self._params, _ = curve_fit(
            CALLABLES[self._callable_key], X, y, loss=self.loss, method="trf", p0=np.ones(X.shape[1] + 1)
        )
        return self
...
class LogLinear(Model):
    ...
    def fit(self, X: NDArray[Any], y: NDArray[Any]) -> Self:
        ...
        super().fit(X, y)
        self._params, _ = curve_fit(
            CALLABLES[self._callable_key], X, np.log(np.clip(y, 1e-6, None)), loss=self.loss, method="trf", p0=np.ones(X.shape[1] + 1)
        )
        return self
...
class ExpNoOffset(Model):
    ...
    def fit(self, X: NDArray[Any], y: NDArray[Any]):
        ...
        self._params, _ = curve_fit(
            CALLABLES[self._callable_key], X, y, loss=self.loss, method="trf", p0=mdl.params
        )
        return self
...
class Exp(Model):
    ...
    def fit(self, X: NDArray[Any], y: NDArray[Any]) -> Self:
        ...
        # Then fit the actual model.
        self._params, _ = curve_fit(
            CALLABLES[self._callable_key], X, y, loss=self.loss, method="trf", p0=p0, bounds=bounds
        )
        return self
...
def fit_model_to_wide_df(
    ...
    try:
        ...
        print(e)
        print(f"no convergance for `{str(model)}` with target `{y_column}`")
        return None
    return model_copy
