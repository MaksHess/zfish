...
def apply_model_to_channel(
    ...
    if model._feature_names == ["Centroid-z"]:
        correction_factors = model._correction_factor(channel_si.coords["z"].expand_dims(["_"], -1))
        if isinstance(correction_factors, np.ndarray):
            correction_factors = xr.DataArray(correction_factors, coords={'z': channel_si.coords['z']})
        return (
            channel_si
            * correction_factors
        ).rename("image")
        ...
        medium_path_si = (~label_si.astype(bool) * label_si.meta.scale_dict["z"]).cumsum(
            dim="z"
        )
