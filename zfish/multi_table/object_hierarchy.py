...
def compute_cardinality_hierarchy(df_hierarchy, df_o):
    ...
    df_cardinality_child = (
        df_hierarchy.filter(pl.col("label.parent") != 0)
    ...
    return (
        df_joined.select(
            ...
            "parents",
            pl.concat_str(
                (pl.col("cardinality_child") > 1)
                .cast(pl.String)
                .replace_strict({"true": "m", "false": "1"}, return_dtype=pl.String),
                (pl.col("cardinality_parent") > 1)
                .cast(pl.String)
                .replace_strict({"true": "m", "false": "1"}, return_dtype=pl.String),
                separator=":",
            ).alias("cardinality"),
