...
from zfish.multi_table.schemas_v2 import IDX_SEL, sel
...
def label_objects_scale_bbx(
    ...
    new_m = _bbx_scaler_to_m(df_m, lowest_level=to_level)
...
def spatial_objects_migrate_bbx(
    df_label_objects: pl.DataFrame,
    df_m: pl.DataFrame,
) -> pl.DataFrame:
    return df_label_objects.join(df_m, on="m").select(
        "o",
        "roi",
        "label",
        *[
            pl.col(f"scale.{dim}").alias(f"{bound}.{dim[1:]}")
            * pl.col(f"m.{dim}.{bound}")
            for bound, dim in product(["lower", "upper"], ["iz", "iy", "ix"])
        ],
    )


def spatial_objects_migrate(
    df_label_objects: pl.DataFrame,
    df_m: pl.DataFrame,
) -> pl.DataFrame:
    return df_label_objects.join(df_m, on="m").select(
...
def _label_objects_scale_bbx(
    ...
    new_m = _bbx_scaler_to_m(
        df_m, from_level=from_level, lowest_level=lowest_level
    ).pipe(debug)
...
def _bbx_scaler_to_m(
    ...
    new_m = (
        df_m.with_columns(
            pl.struct(
                cs.starts_with("scale").gather(from_level) / cs.starts_with("scale")
            )
            .struct.rename_fields([".iz", ".iy", ".ix"])
            .alias("bbx.scaler")
        ).with_columns(
            pl.struct(
                cs.starts_with("scale").gather(lowest_level) / cs.starts_with("scale")
            )
            .struct.rename_fields([".iz", ".iy", ".ix"])
            .alias("bbx.divisible_by")
        )
    ).filter(pl.col("m") <= lowest_level)
    # .select(
    #     sel.idx,
...
def _join_scaler_to_label_objects(
    ...
    return (
        df_label_objects.with_columns(pl.col("m").replace({1: to_level}).cast(pl.UInt8))
        # .join(df_m, left_on="bbx.m", right_on="m")
        .join(df_m, on="m")
        # .pipe(nest, ".", 1)
        # .pipe(debug)
        .select(
            ...
            *[
                (
                    pl.col(f"m.{dim}.{bound}").cast(pl.Float64)
...
def get_quantile_extent(
    ...
    IDIMS = ["iz", "iy", "ix"]
    extent = (
        df_bbxs.join(df_m, on="m")
        .with_columns(
            [
                *[
                    (
                        (pl.col(f"m.{dim}.lower") * pl.col(f"scale.{dim}"))
                        - pl.col(f"scale.{dim}") / 2
                    ).alias(f"lower.{dim.removeprefix('i')}")
                    for dim in IDIMS
                ],
                *[
                    (
                        ((pl.col(f"m.{dim}.upper") - 1) * pl.col(f"scale.{dim}"))
                        + pl.col(f"scale.{dim}") / 2
                    ).alias(f"upper.{dim.removeprefix('i')}")
                    for dim in IDIMS
                ],
            ]
        )
        .with_columns(
            [
                *[pl.lit(0.0).alias(f"origin.{dim}") for dim in DIMS],
                *[
                    (pl.col(f"upper.{dim}") - pl.col(f"lower.{dim}")).alias(
                        f"extent.{dim}"
                    )
                    for dim in DIMS
                ],
        ...
        .select(cs.starts_with("extent").quantile(quantile))
    )
    max_extent = extent.rows()[0]
    if match_length_of_last_two_axes:
        max_extent = tuple(max_extent)[:-2] + (max(max_extent[-2:]),) * 2
    return max_extent


def get_quantile_extent_(
    df_bbxs, quantile=1.0, name_pattern="m.i{dim}", match_length_of_last_two_axes=True
) -> tuple[int, ...]:
    max_extent = (
        df_bbxs.select(
            (
                pl.col(f"{name_pattern.format(dim=dim)}.upper")
                - pl.col(f"{name_pattern.format(dim=dim)}.lower")
            ).alias(f"{dim}.extent")
            for dim in ["z", "y", "x"]
        )
        .quantile(quantile)
...
class LabelImageQueryDask(LazyRasterQueryDask):
    ...
    def lazy(self) -> tuple[da.array, RasterMeta]:
        ...
        if self.resample_target_size is not None:
            assert self.resample_scale_factors is None, (
        ...
        if self.resample_scale_factors is not None:
            assert self.resample_target_size is None, (
                "Only one of `resample_target_size` and `resample_scale_factors` allowed!"
            )
...
def _aggregate_image_paths(
    ...
    if channels is None:
        channels = r.channels["c"]
    if multiscale_level is None:
        multiscale_levels = r.multiscale_levels["m"]
    else:
        multiscale_levels = [multiscale_level]

    df_imgs = (
        (
            r.images.filter(pl.col("c").is_in(channels))
            .filter(pl.col("m").is_in(multiscale_levels))
            .join(
                r.rois.select(
                    pl.col("roi"), pl.col("well"), pl.col("path").name.prefix("roi.")
                ),
                on="roi",
            )
        )
    ...
    df_paths = (
        ...
        .join(
            ...
            left_on=["m", "roi", "mask.o"],
            right_on=["m", "roi", "o"],
            how="left",
        )
        .join(
            r.z_models.select(
                ["c", "z_model", "z_model.o", pl.col("path").name.prefix("z_model.")]
            )
            .with_columns(pl.col("c").cast(pl.Categorical))
            .filter(
                pl.col("z_model").is_null()
                if z_model is None
                else pl.col("z_model") == z_model
            ),
            on="c",
            how="left",
        )
        .join(
            # r.label_images.with_columns(pl.col("o").cast(pl.String)),
            r.label_images.select(
                pl.col("m"),
                pl.col("roi"),
                pl.col("o"),
            ...
            left_on=["m", "roi", "z_model.o"],
            right_on=["m", "roi", "o"],
            how="left",
        )
        .join(
            r.t_models.filter(
                pl.col("t_model").is_null()
                if t_model is None
                else pl.col("t_model") == t_model
            ).select(
                pl.col("c"),
                pl.col("roi"),
                pl.col("t_model"),
...
def _aggregate_label_image_paths(
    ...
    df_label_images = (
        r.label_images.filter(pl.col("o").is_in(object_types))
        .filter(pl.col("m").is_in(multiscale_levels))
        .join(r.rois, on="roi")
    )
...
def _queries_to_images(
    queries: dict[str, LazyRasterQueryDask], drop_meta: bool = False
):
    images = {}
    for name, query in tqdm.tqdm(queries.items()):
        lazy_image = query.compute()
        if isinstance(lazy_image, tuple) and drop_meta:
            lazy_image = lazy_image[0]
        images[name] = lazy_image
    return images
...
class LabelObjectQueryDask(LazyRasterQueryDask):
    object_id: Hashable
    bbx: BoundingBox
    multi_channel_query: MultiChannelQueryDask | DaskImage | NumpyImage
...
def _aggregate_label_object_paths(
    ...
    df_imgs = (
        ...
        .with_columns(
            pl.col("c").is_in(isolate_label_channels).alias("bbx_mask.isolate_label")
        )
        .filter(pl.col("roi").is_in(rois))
    )

    df_label_objects = r.label_objects.filter(pl.col("o") == bbx_object_type).filter(
        pl.col("roi").is_in(rois)
    )
...
def _label_objects_to_queries(
    df_label_objects: pl.DataFrame,
    df_imgs: pl.DataFrame,
    roi_id_columns: tuple[str, ...] = ("roi", "m"),
    object_id_columns: tuple[str, ...] = ("o", "label"),
    ...
    bbx_selector: SelectorType = cs.starts_with("m.i"),
    fuzzy_mask_sigma: int | None = None,
    separator: str = "__",
    ...
    for roi_id_parts, df_imgs_roi in df_imgs.group_by(
        ...
        channel_to_mask_object_type = dict(zip(df_imgs_roi["c"], df_imgs_roi["mask.o"]))
        channel_to_label = {
        ...
        for object_id, bbx_tuple, ch_names, labels in (
            ...
            .agg(
                pl.concat_list(bbx_selector.first()).alias("bbx"),
                pl.col("c"),
                pl.col("label"),
            )
            .rows()
...
def _crop_images_to_size(
    imgs: dict[str, NumpyImage],
    max_extent: tuple[int, ...],  # spatial axis only!
    project: Literal["z", "y", "x"] | None = "z",
) -> dict[str, NDArray]:
    out_imgs = {}
    for name, (img, meta) in imgs.items():
        assert tuple(sorted(meta.dims, key=lambda x: ORDER.index(x))) == tuple(
            meta.dims
        ), f"Dimensions must be in order {ORDER!r}"
        extent = list(img.shape)
        extent[-len(max_extent) :] = max_extent

        if project is not None:
            p_axis = meta.dims.index(project)
            img = img.max(axis=p_axis, keepdims=True)
            extent[p_axis] = 1
            extent = tuple(extent)
        canvas_slice = []
        image_slice = []
        # print(extent, img.shape)
        for extent_canvas, extent_img in zip(extent, img.shape):
            diff = extent_canvas - extent_img
            if diff == 0:
                canvas_slice.append(slice(None, None))
                image_slice.append(slice(None, None))
            elif diff < 0:
                i_slice = slice(abs(diff) // 2, -((abs(diff) + 1) // 2))
                canvas_slice.append(slice(None, None))
                image_slice.append(i_slice)
            else:
                c_slice = slice(diff // 2, -((diff + 1) // 2))
                canvas_slice.append(c_slice)
                image_slice.append(slice(None, None))
            print(image_slice[-1])

        canvas = np.zeros(extent, dtype=img.dtype)
        print(canvas.shape)
        print(canvas_slice, image_slice)
        canvas[tuple(canvas_slice)] = img[tuple(image_slice)]
        print(canvas.shape)
        out_imgs[name] = canvas
    return out_imgs
...
def example_image_query():
    ...
    rl = scan_resources(validate_schema=True)
    r = rl.collect()
    channels = ["DAPI.1", "PCNA.0"]
    channel_masks = [None, None]
    m = 3
    z_model = None
    t_model = None
    dask_chunk_size = (100, 2000, 2000)
    ...
    queries = _images_to_queries(df_imgs, dask_chunk_size=dask_chunk_size)
    res = _queries_to_images(queries)
    return res


def example_lo_query():
    from zfish.multi_table.tables_io import (
        scan_features,
        scan_resources,
    )
    ...
    r = rl.collect()
    channels = ["DAPI.1", "PCNA.0"]
    channel_masks = ["nucleiRaw3", "cells"]
    m = 1
    z_model = None
    t_model = None
    dask_chunk_size = (100, 2000, 2000)
    rois = r.rois.filter(pl.col("well") == "E07")["roi"][:3]
    quantile_extent = 0.95
    print("aggregating paths")
    df_imgs, df_lbl_objs = _aggregate_label_object_paths(
        r,
        "cells",
        channels=channels,
        channel_masks=channel_masks,
    ...
    lblo_queries = _label_objects_to_queries(
        df_imgs=df_imgs,
        df_label_objects=df_lbl_objs,
        fuzzy_mask_sigma=1,
        strategy="memory",
        dask_chunk_size=dask_chunk_size,
    )
    max_extent = get_quantile_extent(df_lbl_objs, quantile_extent)

    print("computing queries")
    lblo_imgs = {query.object_id: query.compute() for query in lblo_queries}

    print("cropping and projecting")
    lblo_cropped = _crop_images_to_size(lblo_imgs, max_extent=max_extent)
    f = (
        scan_features()
        .label.filter(pl.col("o") == "cells")
        .filter(pl.col("roi").is_in(rois))
        .collect()
        .with_columns(pl.lit("1").alias("m"))
        .select(
            pl.concat_str("roi", "m", "o", "label", separator="__").alias("id"),
            "PhysicalSize",
        )
        .sort("PhysicalSize")
    )
    canvas = arrange_on_grid(
        {res_id: res_arr for res_id, res_arr in lblo_cropped.items()},
        order=f["id"].to_list(),
    )

    import napari

    # meta = res["C02_px-1623_py+1916"][1]

    viewer = napari.Viewer()
    viewer.add_image(canvas, channel_axis=1, name=channels)
    return lblo_cropped, f
