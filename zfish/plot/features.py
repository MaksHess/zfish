...
from zfish.features.polars_preprocessing import META_COLUMNS
...
def plot_features_by_embryo(
    ...
    fig, axs = plt.subplots(n_rows, n_cols, figsize=(20, 5 * n_rows), dpi=300, sharex=sharex)
    axs = axs.flatten()
...
def plot_embryos_by_age(df_meta, figsize=(mm_to_inch(100.5), mm_to_inch(68)), dpi=300, **scatterplot_kwrags):
    Y_FEATURE = "log2_nucleiRaw3_Count"
    to_plot = df_meta.sort(Y_FEATURE).with_row_count('rank').to_pandas()

    fig, ax = plt.subplots(figsize=figsize, dpi=dpi)
    plt.plot(
        to_plot['rank'], to_plot[Y_FEATURE], color="k", lw=0.8
    )
    
    default_kwargs = {'data': to_plot, 'x': 'rank', 'y': Y_FEATURE, 'hue': 'cycle', 'palette': 'viridis', 'zorder': 10, 's': 6}
    sns.scatterplot(
        **{
            **default_kwargs,
            **scatterplot_kwrags,
...
def plot_correlation_feature_distributions_per_cycle(df):
    ...
    # df_intensity = df_intensity.idx.join(df_corr.select(sel.index.active | sel.feature())).pipe(pipe_df_nulls).select(pl.all().fill_null(1.0)).pipe(pipe_df_nulls).pipe(unnest_all_structs)
