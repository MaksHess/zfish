...
class OutlierParams:
    ...
    outliers: Sequence[Outlier] = tuple(
            SEGMENTATION_OUTLIERS + INTENSITY_OUTLIERS
    )
    verbose: bool = True
...
class NullHandlingParams:
    strategy: Literal[
        "skip", "drop_feature_rows", "drop_cols", "drop_all"
    ] = "drop_all"
    verbose: bool = True
...
def handle_outliers(df, strategy='drop', outliers: Sequence[Outlier] = tuple(), verbose: bool = True, plot_results: bool=False) -> pl.DataFrame:
    print(outliers)
    if strategy == "skip":
        return df
    elif strategy == "drop":
