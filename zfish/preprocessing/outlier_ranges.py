...
class Outlier(Protocol):
    ...
    def indicate_lower_outlier(self, df: pl.DataFrame) -> pl.Series:
        self.fit(df)
        col_expression = pl.col(self.feature).map_elements(TRANSFORMS[self.transform])
        if self.lower is None:
    ...
    def indicate_outlier(self, df: pl.DataFrame) -> pl.Series:
        return (
            self.indicate_lower_outlier(df)
            | self.indicate_upper_outlier(df)
        ).to_frame()
...
class IQROutlier(Outlier):
    ...
    def fit(self, df):
        if not self.transform == 'identity':
            raise NotImplementedError("IQROutlier on transformed feature columnes not yet implemented!")
...
def clip_outliers(df, outliers: Sequence[Outlier], verbose=True, plot_results=False):
    ...
    for outlier in outliers:
        ...
        if outlier.lower is not None:
            df_out = df_out.with_columns(pl.col(outlier.feature).clip_min(outlier.lower))
        if outlier.upper is not None:
            df_out = df_out.with_columns(pl.col(outlier.feature).clip_max(outlier.upper))
    if plot_results:
        results_plot(df, df_out, outliers)
    return df_out
...
def main():
    ...
    log_outlier = RngOutlier(feature='a', lower=np.log(3), upper=np.log(6), transform='log')
    log10_outlier = RngOutlier(feature='a', lower=np.log10(3), upper=np.log10(6), transform='log10')
    log2_outlier = RngOutlier(feature='a', lower=np.log2(3), upper=np.log2(6), transform='log2')
